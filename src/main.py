...
def main():
    ...
    velocity_threshold = 20  # In degrees/sec. Adjust this threshold as needed
    df = process_data(preprocessed_data_path, processed_data_path, velocity_threshold)
    print(df['type'].value_counts()['saccade'])
    plot_saccades(df)
    #plot_fixations(df)
